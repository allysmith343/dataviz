class TidyTree {
  ...
  initVis() {
    ...
    vis.svg = d3.select(vis.config.parentElement).append('svg')
        ...
        //.attr('height', vis.config.height + vis.config.margin.top + vis.config.margin.bottom);
  ...
  renderVis() {
    ...
    const node = vis.chart.selectAll('g')
        ...
        .join('g')
        .attr('transform', d => `translate(${vis.config.width-d.y},${d.x})`);

    node.append('circle')
        .attr('class', 'node-circle')
        .attr('r', 5.5)
        .style("fill", function(d){ return d.data.gender == "Female" ? 'violet' : 'pink'});
    ...
    node
      .on('mouseover', (event,d) => {
        d3.select('#tooltip')
          .style('display', 'block')
          .html(`<div class="tooltip-label"><b>${d.data.name}</b></div>
          <div><i>Birthday: ${d.data.birthday}</i></div>
          `);
    });
  }
}